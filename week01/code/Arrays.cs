public static class Arrays
{
    ...
    public static double[] MultiplesOf(double number, int length)
    {
        ...
        //The first step to solve this problem must be to create the array were the resulting multiples will be stored with a capacity of "lenght" so that it is able to store the amount of multiples requested, and then start a loop that will run as many times as "length". Every time the loop is run, the int "number" will be multiplied by "i" (which is initialized with a value of 1 and increases with every iteration) and the resulting multiple will be added to the array. once the loop is done, the array with multiples will be returned.
